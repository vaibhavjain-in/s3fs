...
function _s3fs_upload_test($filename) {
  ...
  _s3fs_assert($file_count == 2, __LINE__, "There should be exactly 2 files in the s3://_s3fs_test directory. Instead, there are $file_count.");
  ...
  $moved_uri = 's3://_s3fs_test/test_file-moved.jpg';
  $ret = file_move($s3_file, $moved_uri, FILE_EXISTS_REPLACE);
  _s3fs_assert($ret, __LINE__, 'Failed to move the file to test_file-moved.jpg');
  $ret = file_exists('s3://_s3fs_test/test_file.jpg');
  _s3fs_assert($ret == FALSE, __LINE__, 'The old file should no longer exist.');
  ...
  _s3fs_assert(drupal_unlink($uploaded_uri), __LINE__, 'The delete call for the first file failed.');
  _s3fs_assert(drupal_unlink($uploaded_uri2), __LINE__, 'The delete call for the second file failed.');
